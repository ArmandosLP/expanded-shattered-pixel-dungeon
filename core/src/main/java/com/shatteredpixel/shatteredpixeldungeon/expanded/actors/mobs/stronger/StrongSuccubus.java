...
import com.shatteredpixel.shatteredpixeldungeon.expanded.actors.mobs.Incubus;
...
public class StrongSuccubus extends Mob {

	private int blinkCooldown = 0;
    private int incubusID = -1;
    ...
    protected boolean act() {
        ...
        if (incubusID == -1){
            ...
            if (!candidates.isEmpty()){
                ...
                incubus.succubusID = this.id();
                this.incubusID = incubus.id();

                if (state != SLEEPING) {
                    incubus.state = incubus.WANDERING;
                }

                incubus.pos = Random.element( candidates );

                GameScene.add( incubus );
                Dungeon.level.occupyCell(incubus);

                if (sprite.visible) {
                    Actor.add( new Pushing( incubus, pos, incubus.pos ) );
                }

                //champion buff, mainly
                for (Buff b : buffs()){
                    if (b.revivePersists) {
                        Buff.affect(incubus, b.getClass());
                    }
                }
    ...
    private class Sleeping extends Mob.Sleeping {
        ...
        public boolean act( boolean enemyInFOV, boolean justAlerted ) {
            Incubus partner = (Incubus) Actor.findById( incubusID );
    ...
    // FIXME Not a good solution but works for now
    // Used exclusive to give incubus access to a protected value outside the package
