...
public class StrongGhoul extends Mob {
	...
	protected boolean act() {
		...
		if (partnerID == -1){
			...
			if (!candidates.isEmpty()){
				StrongGhoul child = new StrongGhoul();
	...
	public void die(Object cause) {
		if (cause != Chasm.class && cause != GhoulLifeLink.class && !Dungeon.level.pit[pos]){
			StrongGhoul nearby = GhoulLifeLink.searchForHost(this);
			if (nearby != null){
    ...
    public void damage(int dmg, Object src) {
        ...
        for ( Mob mob : Dungeon.level.mobs.toArray(new Mob[0]) ){
            if (
                    mob instanceof StrongGhoul &&
                            mob != this &&
                            fieldOfView[mob.pos]
            ){
                WillingSacrifice ng =  mob.buff(WillingSacrifice.class);
                if ( ng != null ) ng.extend(dmg);
                else Buff.affect(mob, WillingSacrifice.class).set(dmg,1);
                ...
                if (sprite.visible || mob.sprite.visible){
                    MagicMissile.boltFromChar(
                            ...
                            new Callback() {
                                @Override
                                public void call() {
                                    SpellSprite.show(mob, SpellSprite.WILLING_SACRIFICE);
                                    next();
                                }
                            });
                }
    ...
    public String description() {
        return Messages.get(Ghoul.class, "desc") + "\n\n" + Messages.get(this, "talent");
    }

    @Override
    public String name() {
        return Messages.get(Ghoul.class, "name");
    }
	...
	private class Sleeping extends Mob.Sleeping {
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			StrongGhoul partner = (StrongGhoul) Actor.findById( partnerID );
	...
	public static class GhoulLifeLink extends Buff{
		...
		public void set(int turns, StrongGhoul ghoul){
			this.ghoul = ghoul;
			turnsToRevive = turns;
		}
		...
		public void detach() {
			super.detach();
			StrongGhoul newHost = searchForHost(ghoul);
			if (newHost != null){
				attachTo(newHost);
				timeToNow();
			} else {
		...
		public void restoreFromBundle(Bundle bundle) {
			super.restoreFromBundle(bundle);
			ghoul = (StrongGhoul) bundle.get(GHOUL);
			ghoul.beingLifeLinked = true;
			turnsToRevive = bundle.getInt(LEFT);
		}

		public static StrongGhoul searchForHost(StrongGhoul dieing){

			for (Char ch : Actor.chars()){
				//don't count hero ally ghouls or duel frozen ghouls
				if (ch != dieing && ch instanceof StrongGhoul
    ...
    public static class WillingSacrifice extends Buff {
        ...
        public void set(int duration, int level) {
            this.left = duration;
            this.level = level;
        }
        ...
        @Override
        public int icon() {
            return BuffIndicator.WILLING_SACRIFICE;
        }

        @Override
        public boolean act() {
            if (target.HP < target.HT) {
                int heal = Math.min(target.HT - target.HP, level);
                if (heal > left) heal = left;
                target.HP = Math.min(target.HT, target.HP + heal);
            }

            //Show the healing amount even when no actual healing, so the player knows the effect is being reduced even at full health.
            target.sprite.showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(level), FloatingText.HEALING);
